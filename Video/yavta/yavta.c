...
{
printf("shashi:v4l2_buf_type_name\n");
	static struct {
		enum v4l2_buf_type type;
		const char *name;
	} names[] = {
...
{
	printf("shashi:v4l2_fourcc_name\n");	
	static char name[5];
	unsigned int i;
...
static int video_open(struct device *dev, const char *devname, int no_query)
{
	...
	if (!no_query) {
		...
		printf("shashi:video_open:VIDIOC_QERYCAP\n");
...
{
	printf("shashi:video_close:before:free(dev->pattern)\n");
	free(dev->pattern);
	printf("shashi:video_close:before:free(dev->buffers)\n");
	free(dev->buffers);
	printf("shashi:video_close:before:close(dev->fd)\n");
	close(dev->fd);
}
...
static void uvc_get_control(struct device *dev, unsigned int id)
{
	...
	ctrl.id = id;
	printf("shashi:uvc_get_control:VIDIOC_G_CTRL\n");
...
static void uvc_set_control(struct device *dev, unsigned int id, int value)
{
	...
	ctrl.id = id;
	ctrl.value = value;
	printf("shashi:uvc_set_control: VIDIOC_S_CTRL\n");
...
static int video_get_format(struct device *dev)
{
	...
	fmt.type = dev->type;
	printf("shashi:video_get_format:VIDIOC_G_FMT\n");
...
static int video_set_framerate(struct device *dev, struct v4l2_fract *time_per_frame)
{
	...
	parm.type = dev->type;
	printf("shashi:video_set_framerate:VIDIOC_G_PARAM\n");
...
static void video_query_menu(struct device *dev, unsigned int id)
{
	...
	while (1) {
		menu.id = id;
		printf("shashi:video_query_menu:VIDIOC_QUERYMENU\n");
...
{
printf("shashi:video_list_controls\n");
	...
	while (1) {
...
#endif
		printf("shashi:video_list_controls:VIDIOC_QUERYCTRL\n");
		...
		ctrl.id = query.id;
		printf("shashi:video_list_controls:VIDIOC_G_CTRL\n");
		ret = ioctl(dev->fd, VIDIOC_G_CTRL, &ctrl);
		if (ret < 0)
			strcpy(value, "n/a");
		else
	...
	unsigned int width, unsigned int height)
{
	...
	for (i = 0; ; ++i) {
		...
		ival.width = width;
		ival.height = height;
		printf("shashi:video_enum_frame_intervals:VIDIOC_ENUM_FRAMEINTERVALS\n");
...
static void video_enum_frame_sizes(struct device *dev, __u32 pixelformat)
{
	...
	for (i = 0; ; ++i) {
		...
		printf("shashi:video_enum_frame_sizes:VIDIOC_ENUM_FRAMESIZES\n");
...
static void video_enum_formats(struct device *dev, enum v4l2_buf_type type)
{
	...
	for (i = 0; ; ++i) {
		...
		fmt.index = i;
		fmt.type = type;
		printf("shashi:video_enum_formats:VIDIOC_ENUM_FMT\n");
...
static void video_enum_inputs(struct device *dev)
{
	...
	for (i = 0; ; ++i) {
		...
		input.index = i;
		printf("shashi:video_enum_inputs:VIDIOC_ENUM_INPUTS\n");
...
static int video_get_input(struct device *dev)
{
	__u32 input;
	int ret;
	printf("shashi:video_get_input:VIDIOC_G_INPUT\n");
...
static int video_set_input(struct device *dev, unsigned int input)
{
	__u32 _input = input;
	int ret;
	printf("shashi:video_set_input:VIDIOC_S_INPUT\n");
...
static int video_set_quality(struct device *dev, unsigned int quality)
{
	...
	printf("shashi:video_set_quality:VIDIOC_S_JPEGCOMP\n");
	...
	printf("shashi:video_set_quality:VIDIOC_G_JPEGCOMP\n");
...
{
printf("shashi:video_load_test_pattern\n");	
...
printf("shashi:main:video_open\n");
	...
	if (do_get_control){
printf("shashi:main:vvc_get_control\n");
...
printf("shashi:main:vvc_get_control\n");
	...
	if (do_list_controls){
printf("shashi:main:do_list_control\n");
...
printf("shashi:main:video_set_format:video_close\n");
			video_close(&dev);
			return 1;
		}
	}
...
printf("shashi:main:video_set_framerate:video_close\n");
			video_close(&dev);
			return 1;
		}
	}
	...
	if (!do_capture) {
printf("shashi:main:!do_capture:video_close\n");
		video_close(&dev);
		return 0;
	}
...
printf("shashi:main:video_set_quality:video_close\n");
		video_close(&dev);
		return 1;
	}

	if (video_prepare_capture(&dev, nbufs, userptr_offset, filename)) {
printf("shashi:main:video_prepare_capture:video_close\n");
		video_close(&dev);
		return 1;
	}
...
printf("shashi:main:video_do_capture:video_close\n");
		video_close(&dev);
		return 1;
	}
printf("shashi:main:before:video_close\n");
	video_close(&dev);
	return 0;
}
